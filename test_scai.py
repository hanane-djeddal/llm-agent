...
RAGAGENT_MODEL_NAME = "hanane/zephyr-hagrid-deduplicated-rag-agent-3b"  # "hanane/zephyr-hagrid-rag-agent-3b"  # (
#     "erbacher/zephyr-rag-agent-webgpt"  # "erbacher/zephyr-rag-agent"  #
# )
...
def test():
    config = PeftConfig.from_pretrained(RAGAGENT_MODEL_NAME, load_in_8bit=True)
    model = AutoModelForCausalLM.from_pretrained(
        "stabilityai/stablelm-zephyr-3b", device_map="auto", force_download=True
    )
    #     "HuggingFaceH4/zephyr-7b-beta", device_map="auto", force_download=True
    # )
    ...
    kwargs = {"do_sample": True, "top_p": 0.5, "max_new_tokens": 1000}
    docs, scores, answer = agent.generate(
        "Who has the highest goals in men's world international football?", **kwargs
