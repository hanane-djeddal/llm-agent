...
def test_alce_docs_gtr():
    ...
    for itera, row in enumerate(tqdm(dataset)):
        ...
        if (itera+1) % 150 == 0:
            ...
            results_file = results_path+"intr_testasqa_"+tag+"_"+str(args.nb_rounds)+"rounds_"+str(args.nb_docs)+"docs.json"  # "agent_hagrid_3doc_2rounds.csv"
    ...
    results_file = (
        ...
        if args.output_dir
            ...
            results_path+"all_testasqa_"+tag+"_"+str(args.nb_rounds)+"rounds_"+str(args.nb_docs)+"docs.json" 
        )
    )
    with open(results_file, "w") as writer:
        json.dump(results_df, writer)
